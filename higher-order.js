...
//Code Here
let totalPopulation = populations.reduce(reducer = (total, nextNum) => total + nextNum);
...
*/
let totaler = (price, tax) => price*tax;
let orderTotals = orders.map(x => x = x.price+ x.price * x.tax); // Code here
...
let bobsTotal = purchases.filter(x => x.owner == "Bob").reduce((a, b) => ({price: a.price + b.price})).price;//Code Here